...
export default async function handler(req: NextApiRequest, res: NextApiResponse<OkPayload>) {
  ...
  if (!clubMillasSecret) {
    return res.status(503).json({
      ok: false,
      message: "Variable CLUB_MILLAS_POS_SECRET no definida en el servidor del POS.",
    });
  }
  ...
  const wmsBody: Record<string, unknown> = {
    ventaId: str(b.ventaId) || `pos-${Date.now()}`,
    puntoVenta,
    montoTotalCop,
  ...
  if (!qrPayload) {
    return res.status(200).json({
      ok: false,
      message: "El WMS no devolvió qrPayload para acumular millas.",
    });
  }
